...
class _BusinessViewByCategoryState extends State<BusinessViewByCategory> {
                            ...
                            Row(
                              children: <Widget>[
                                Container(
                                    ...
                                    child: Text(
                                      businessModel.averageRating.toString() +
                                          ".0",
                                ...
                                SmoothStarRating(
                                    ...
                                    rating:
                                        businessModel.averageRating.toDouble(),
  ...
  callApiForAddToFavourite(BuildContext context, Business businessModel) {
    NetworkCall<AddFavoriteRes>()
        .makeCall(
            () => app
                .resolve<ServiceModule>()
                .networkService()
                .addToFovourite(businessModel.sId),
            context,
            isProgress: true)
        .then(
      (addFavoriteRes) async {
        showToast("Added into favorite list...", context: context);
        setState(() {});
      },
    ).catchError((onError) {
      showToast("sorry! try again...", context: context);
      setState(() {});
    });
  }
